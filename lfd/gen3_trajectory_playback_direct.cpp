...
class gen3_traj_replay
{
      ...
      if (not_interpolate_first){
        ...
        for (int ind = 0; ind < traj_array.rows(); ind++)
        {
          ...
          if (interpolate)
          {
            ...
            auto dist = (time_increment.toSec() / 0.001);

            if(use_time){
              auto dist = (time_mat(ind+1, time_mat.cols()-1)-time_mat(ind, time_mat.cols()-1) / 0.001); // decrease the last number to get valid trajectory
            }
  ...
  //label:
      ...
      for (int i = 0; i < num_waypoints; i++)
      {
        ...
        moveit_trajectory.addSuffixWayPoint(state, time_increment.toSec());   // it was 0.0, changed to 0.001
      }
      ...
      for (int i = 0; i < num_waypoints; i++)
      {
        smooth_traj_msg.joint_trajectory.points[i].time_from_start = ros::Duration(time_increment*i);
      }
    ...
    bool gen3_play()
    {
      ...
      trajArrayToJointTrajectory_moveit(joint_names, traj_array, robot_model,use_sampled_time, true, true, t);
      ...
      // execution_client.waitForResult(ros::Duration(5.0));
