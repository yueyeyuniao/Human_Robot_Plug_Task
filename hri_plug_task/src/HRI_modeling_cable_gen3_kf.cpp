...
const double tip_length = 0.10;
const bool filter_plug = true;
bool socket_stable = false;
...
class PointCloudProc
{
  public:
                       ...
                       cloud_hull(new pcl::PointCloud<pcl::PointXYZ>), cloud_raw_(new pcl::PointCloud<pcl::PointXYZ>)
    {
        ...
        pc_sub_ = nh_.subscribe("/camera_top/depth_registered/points", 1, &PointCloudProc::pointcloudcb, this);
    ...
    label:
        ...
        // check the socket pose, if read a stable pose, mark the socket_stable as true and skip the check
        // if (socket_stable == false) {
        //   listener_.waitForTransform("/base_link", "socket_updated",ros::Time(0), ros::Duration(3.0)); 
        //   listener_.lookupTransform("/base_link", "socket_updated",ros::Time(0), transform_socket);
        ...
        pass.setFilterLimits(transform.getOrigin().x()+0.08, center_object.x + transform_socket.getOrigin().x());
        
        pass.filter(*cloud_filtered_);
        pass.setInputCloud(cloud_filtered_);
        pass.setFilterFieldName("y");
        pass.setFilterLimits(center_object.y - 0.1, center_object.y + transform.getOrigin().y()+0.4);
        pass.filter(*cloud_filtered_);
        pass.setInputCloud(cloud_filtered_);
        pass.setFilterFieldName("z");
        pass.setFilterLimits(center_object.z + 0.1, center_object.z + 0.7);  
        pass.filter(*cloud_filtered_);
